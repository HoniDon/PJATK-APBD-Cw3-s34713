...
public sealed class ZadaniaLinq
{
    ...
    {
        var studentsFromWarsaw = DaneUczelni.Studenci
    ...
    {
        var sortedStudents = DaneUczelni.Studenci
    ...
    {
        var firstAnalyticSubject = DaneUczelni.Przedmioty
            .FirstOrDefault(s => s.Kategoria == "Analytics");

        if (firstAnalyticSubject == null)
        {
            return new[] { "Brak przedmiotu Analytics" };
        }
        
        return new[] { $"{firstAnalyticSubject.Nazwa} {firstAnalyticSubject.DataStartu}" };
    }
    ...
    {
        bool existsInactiveRecord = DaneUczelni.Zapisy
            .Any(z => !z.CzyAktywny);
        
        return new[] { existsInactiveRecord ? "True" : "False" };
    }
    ...
    {
        bool allLecturersHaveDepartments = DaneUczelni.Prowadzacy
            .All(p => !string.IsNullOrWhiteSpace(p.Katedra));
        
        return new[] { allLecturersHaveDepartments ? "True" : "False" };       
    }
    ...
    {
        int activeRecordsCount = DaneUczelni.Zapisy
            .Count(z => z.CzyAktywny);
        
        return new[] { activeRecordsCount.ToString() };
    }
    ...
    {
        var distinctCities = DaneUczelni.Studenci
            .Select(s => s.Miasto)
            .Distinct()
            .OrderBy(c => c);
    ...
    {
        var secondPage = DaneUczelni.Przedmioty
            .OrderBy(p => p.Nazwa)
            .Skip(2)
            .Take(2)
    ...
    {
        var studentsWithRecords = DaneUczelni.Studenci
    ...
    {
        var groupedRecords = DaneUczelni.Zapisy
            .Join(
                DaneUczelni.Przedmioty, 
                z => z.PrzedmiotId, 
                p => p.Id, 
    ...
    {
        var averageScores = DaneUczelni.Zapisy
            .Join(
                DaneUczelni.Przedmioty, 
                z => z.PrzedmiotId, 
                p => p.Id, 
                (z, p) => new { Przedmiot = p, Zapis = z })
            .Where(zs => zs.Zapis.OcenaKoncowa != null)
            .GroupBy(zs => zs.Przedmiot.Nazwa)
            .Select(g => $"{g.Key} {g.Average(zs => zs.Zapis.OcenaKoncowa)}");
    ...
    {
        var lecturersWithSubjectsCounts = DaneUczelni.Prowadzacy
            .GroupJoin(
                DaneUczelni.Przedmioty,
                pr => pr.Id,
                ps => ps.ProwadzacyId,
                (pr, ps) => $"{pr.Imie} {pr.Nazwisko} {ps.Count()}");
        
        return lecturersWithSubjectsCounts;      
    }
    ...
    {
        var highestScores = DaneUczelni.Studenci
            .Join(
                DaneUczelni.Zapisy,
                s => s.Id,
                z => z.StudentId,
                (s, z) => new { Student = s, Zapis = z })
            .Where(zs => zs.Zapis.OcenaKoncowa != null)
            .GroupBy(zs => zs.Student.Imie + " " + zs.Student.Nazwisko)
            .Select(g => $"{g.Key} {g.Max(zs => zs.Zapis.OcenaKoncowa)}");
    ...
    {
        var studentsWithMoreThanOneActiveSubject = DaneUczelni.Studenci
            .Join(
                DaneUczelni.Zapisy,
                s => s.Id,
                z => z.StudentId,
                (s, z) => new { Student = s, Zapis = z })
            .Where(zs => zs.Zapis.CzyAktywny)
            .GroupBy(zs => $"{zs.Student.Imie} {zs.Student.Nazwisko}")
            .Where(g => g.Count() > 1)
            .Select(g => $"{g.Key} {g.Count()}");
        
        return studentsWithMoreThanOneActiveSubject;      
    }
    ...
    {
        var subjectsStartingInApril = DaneUczelni.Przedmioty
            .Where(p => p.DataStartu.Month == 4 && p.DataStartu.Year == 2026)
            .Join(
                DaneUczelni.Zapisy, 
                p => p.Id, 
                z => z.PrzedmiotId, 
                (p, z) => new { Przedmiot = p, Zapis = z })
            .GroupBy(zs => zs.Przedmiot.Nazwa)
            .Where(g => g.All(zs => zs.Zapis.OcenaKoncowa == null))
            .Select(g => g.Key);       
        
        return subjectsStartingInApril;      
    }
    ...
    {
        var activeRecordsCountByCity = DaneUczelni.Studenci
            .Join(
                DaneUczelni.Zapisy,
                s => s.Id,
                z => z.StudentId,
                (s, z) => new { Student = s, Zapis = z })
            .Where(zs => zs.Zapis.CzyAktywny)
            .GroupBy(zs => zs.Student.Miasto)
            .OrderByDescending(g => g.Count())
            .Select(g => $"{g.Key} {g.Count()}");
        
        return activeRecordsCountByCity;      
    }
